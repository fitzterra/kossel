...
#define STEPS_PER_TURN       (8*400)  // depends on your stepper motor.  most are 200.
//#define STEPS_PER_MM         (STEPS_PER_TURN*16/1.25)  // (400*16)/1.25 with a M8 spindle
#define STEPS_PER_MM         (STEPS_PER_TURN/38.33)  // 38.33=GT2 circumference
#define MAX_FEEDRATE         (1000000) // Steps per second = 1 step per microsecond
#define MIN_FEEDRATE         (1)
#define NUM_AXIES            (4)
// Define this if you have a min as well as a max limit switch per axis. Then also
// Ensure you set the pins correctly in motor_setup()
#define DUAL_LIMIT
  ...
  for( i=0; i<maxsteps; ++i ) {
    for(j=0;j<NUM_AXIES;++j) {
      ...
      if(a[j].over >= maxsteps) {
        ...
        delayMicroseconds(20);  // Minimum 20usecs step pulse width for safety
...
void motor_setup() {
    ...
    */
    ...
    // This is for the Geeetech GT2560 board
    // X Motor
    motors[0].step_pin=25;
    motors[0].dir_pin=23;
    motors[0].enable_pin=27;
    motors[0].limit_min_pin=22;
    motors[0].limit_max_pin=24;
    // Y Motor
    motors[1].step_pin=31;
    motors[1].dir_pin=33;
    motors[1].enable_pin=29;
    motors[0].limit_min_pin=26;
    motors[0].limit_max_pin=28;
    // Z Motor
    motors[2].step_pin=37;
    motors[2].dir_pin=39;
    motors[2].enable_pin=35;
    motors[0].limit_min_pin=30;
    motors[0].limit_max_pin=32;
    // Extruder motor - designated by A in schematic
    motors[3].step_pin=35;
    motors[3].dir_pin=37;
    motors[3].enable_pin=36;
    motors[0].limit_min_pin=0;
    motors[0].limit_max_pin=0;
...
void setup() {
  ...
  feedrate(4000);  // set default speed
  ready();
}
